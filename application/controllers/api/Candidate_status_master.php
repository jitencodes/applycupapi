...
class Candidate_status_master extends BD_Controller{
    function __construct()
    {
        // Construct the parent class
        parent::__construct();
        $this->load->model('api/CandidateStatusModel','CandidateStatus');
        $this->auth();
    }
    ...
    public function index_get(){
        ...
        $total = $this->CandidateStatus->count_all();
        $data = $this->CandidateStatus->get_list($id,$search,$start,$length,$status);
        if (!empty($data))
        {
    ...
    public function index_post(){
        try {
            ...
            if ($this->CandidateStatus->is_exist($name)){
                $this->response([
                    'status' => False,
                    'message' => "This status already exits",
                ], REST_Controller::HTTP_BAD_REQUEST);
            }
            if ($this->CandidateStatus->add($name,$ordering,$status)) {
                $this->response([
                    'status' => True,
    ...
    public function index_put()
    {
        try {
            ...
            if ($this->CandidateStatus->is_exist($name,$id)){
                $this->response([
                    'status' => False,
                    'message' => "This status already exits",
                ], REST_Controller::HTTP_BAD_REQUEST);
            }
            if ($this->CandidateStatus->update($id,$name,$ordering,$status)) {
                $this->response([
                    'status' => True,
