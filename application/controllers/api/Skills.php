...
class Skills extends BD_Controller
{
    function __construct()
    {
        // Construct the parent class
        parent::__construct();
        $this->load->model('api/SkillModel','Skills');
        $this->auth();
    }
    ...
    public function index_get(){
        ...
        $total = $this->Skills->count_all();
        $data = $this->Skills->get_skills($id,$search,$start,$length,$status);
        if (!empty($data))
        {
    ...
    public function index_post(){
        try {
            ...
            $name = $this->post('skill');
            $ordering = $this->post('ordering');
            $status = $this->post('status');
            if ($this->Skills->is_exist($name)){
                $this->response([
                    'status' => False,
                    'message' => "This skill already exits",
                ], REST_Controller::HTTP_BAD_REQUEST);
            }
            $res = $this->Skills->add($name,$ordering,$status);
            if ($res) {
                $this->response([
                    'status' => True,
                    'id' => $res,
    ...
    public function index_put()
    {
        try {
            ...
            $this->form_validation->set_rules('id', 'Skill Id', 'trim|required');
            $this->form_validation->set_rules('skill', 'Skill Name', 'trim|required');
            $this->form_validation->set_rules('ordering', 'Ordering', 'trim|required');
            ...
            if ($this->Skills->is_exist($name,$id)){
                $this->response([
                    'status' => False,
                    'message' => "This skill already exits",
                ], REST_Controller::HTTP_BAD_REQUEST);
            }
            if ($this->Skills->update($id,$name,$ordering,$status)) {
                $this->response([
                    'status' => True,
